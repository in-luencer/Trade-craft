import { IndicatorCondition, IndicatorParams, Strategy } from "./types"
...
export function getIndicatorLogic(condition: IndicatorCondition): string {
  ...
  if (logic === "crosses_above" || logic === "crosses_below") {
    if (typeof value === "string" && value.startsWith("indicator:")) {
      ...
      return `${getIndicatorDisplayName(condition)} ${logicDescription} ${targetIndicatorDisplayName}`;
    }
  }
  ...
  const logicMap: Record<string, string> = {
    ...
    percent_change: "percent change is", // Needs value appended
    turns_up: "turns up",
  ...
  if (["greater_than", "less_than", "equals", "percent_change"].includes(logic)) {
     return `${getIndicatorDisplayName(condition)} ${logicDescription} ${value}`;
  }

  return `${getIndicatorDisplayName(condition)} ${logicDescription}`;
}

export function getIndicatorParams(condition: IndicatorCondition, indicatorType: string): IndicatorParams {
...
export function collectIndicators(strategy: Strategy): Set<string> {
