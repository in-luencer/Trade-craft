...
export default function IndicatorLogicEngine({ condition, onChange, onRemove }: IndicatorLogicEngineProps) {
  ...
  // Initialize parameter values and condition.value from metadata defaults
  useEffect(() => {
    if (indicator) {
      ...
      onChange({
        ...
        value: condition.value || indicator.logicOptions.find(opt => opt.value === condition.logic)?.defaultValue?.toString() || "0"
      })
    }
  }, [condition.indicator])
